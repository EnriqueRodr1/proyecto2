...
def buscar_socio(data: LoginInput, db: Session = Depends(get_db)):
    socio = db.query(models.Socio).filter(models.Socio.email == data.email).first()
    if not socio or socio.contrasenya != data.contrasenya:
